...
fs.readFile('../textos/texto-gobernador.txt', 'utf8', function (err,data) {
  if (err) {
  ...
  fs.writeFile("../salida/gobernadoresBA.csv", resp.join('\n'), function(err) {
        if(err) {
            return console.log(err);
        }

        console.log("La salida fue guardad en salida/gobernadoresBA.csv");
    }); 
});
