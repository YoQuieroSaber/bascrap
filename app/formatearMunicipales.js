...
var analyzer = function(lines){
    ...
    var resp    = [[ '"' + "Partido","Linea Interna", "Seccion", "Distrito", "Cargo",
    ...
    function distrito(actual, i){
        while(!lines[i++].match(Estados.distrito));
        actual.distrito = lines[i -1].match(Estados.distrito)[1].trim();
        return i;
    }

    function imprimirFila(actual){
        var linea = [actual.partido, actual.lineaInterna, actual.seccion, actual.distrito,
        actual.cargo, 
...
fs.readFile('../textos/texto-muni.txt', 'utf8', function (err,data) {
  if (err) {
  ...
  fs.writeFile("../salida/municipalesBA.csv", resp.join('\n'), function(err) {
        if(err) {
            return console.log(err);
        }

        console.log("La salida fue guardad en salida/municipalesBA.csv");
    }); 
});
